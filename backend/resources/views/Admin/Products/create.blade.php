...
@section('styles')
    ...
    <style>
        ...
        .product-save-modal-card h3 {
            margin: 15px 0 8px;
            color: var(--theme-text-main);
        }

        .product-save-modal-card p {
            margin: 0;
            color: var(--theme-text-gray);
            line-height: 1.55;
        }

        .product-save-modal-actions {
            display: flex;
        ...
        .product-save-modal-actions button {
            min-width: 100px;
        }
        ...
        .btn-product-draft-ai:disabled {
            cursor: wait;
            opacity: .65;
        }

        .product-draft-ai-status {
            margin-top: 12px;
            padding: 9px 10px;
            border-radius: 7px;
            color: #a8440d;
        ...
        .product-draft-ai-status.error {
            color: #a12626;
            background: #fff1f1;
        }

        .product-draft-ai-result {
            display: grid;
            gap: 8px;
            margin-top: 12px;
        }

        .product-draft-ai-item {
            padding: 10px;
        ...
        .variant-missing-combinations[hidden] {
            display: none;
        }
        ...
        .variant-editor-card:not(.is-collapsed) .variant-card-toggle::after {
            transform: rotate(180deg);
        }

        .variant-editor-card.is-collapsed .variant-card-details {
            display: none;
        }
        ...
        .variant-card-status.active {
            ...
            background: rgba(19, 178, 77, 0.77);
        }

        .variant-card-status.inactive {
            color: var(--theme-text-gray);
        ...
        .bottom-fixed-actions-bar {
            ...
            bottom: 0;
            left: 260px;
            /* Aligns perfectly with layout sidebar width */
            right: 0;
            background-color: #ffffff;
            border-top: 1px solid var(--theme-border);
            padding: 16px 32px;
...
@section('content')
    ...
    <form id="product-edit-form"
        action="{{ $isCreate ? route('admin.products.store') : route('admin.products.update', $product->id) }}"
        ...
        <div class="create-listing-wrapper">
            ...
            <div class="layout-column-main">
                ...
                <div class="form-card">
                    ...
                    <div class="form-control-row product-name-slug-row">
                        ...
                        <div class="form-control-group no-margin">
                            ...
                            <div class="slug-input-wrap">
                                ...
                                <button type="button" id="regenerate-slug" class="slug-regenerate-button"
                                    title="Tạo lại slug từ tên">
                                    <i class="fa-solid fa-rotate"></i>
                                </button>
                            </div>
                            <div class="slug-field-note">
                                {{ $isCreate ? 'Tự tạo từ tên sản phẩm; bạn có thể chỉnh sửa trước khi tạo.' : 'Slug cũ được giữ để chuyển hướng SEO khi thay đổi.' }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-control-group" style="margin-top: 14px;">
                        <div class="seo-field-heading">
                            ...
                            <span class="seo-counter-status"><span class="seo-limit-warning" data-limit-warning>Quá ký
                                    tự</span><span id="short-description-count"
                                    class="seo-character-count">0/300</span></span>
                        </div>
                        <textarea id="short_description" name="short_description"
                            class="input-text-field short-description-field" maxlength="500"
                ...
                @if($isCreate)
                    <section class="form-card product-draft-ai-card" id="product-draft-ai-card">
                        <div class="form-card-title"><i class="fa-solid fa-wand-magic-sparkles"></i><span>Tạo nháp sản phẩm bằng
                                AI</span></div>
                        <p class="product-draft-ai-note">Nhập tên sản phẩm chính, AI sẽ tạo bản nháp để bạn kiểm tra. Chưa có dữ
                            liệu nào được lưu.</p>
                        <button type="button" id="btn-generate-product-draft" class="btn-product-draft-ai">Tạo nháp từ tên sản
                            phẩm</button>
                        <button type="button" id="btn-generate-draft-image-alt" class="btn-product-draft-ai">Tạo alt ảnh bằng
                            AI</button>
                        <div id="product-draft-ai-status" class="product-draft-ai-status" aria-live="polite" hidden></div>
                        <div id="product-draft-ai-result" class="product-draft-ai-result" hidden></div>
                    </section>
                @endif
                ...
                <div class="form-card">
                    ...
                    <div class="form-control-group">
                        <div class="seo-field-heading">
                            ...
                            <span class="seo-counter-status"><span class="seo-limit-warning" data-limit-warning>Quá ký
                                    tự</span><span id="seo-title-count" class="seo-character-count">0/60</span></span>
                        </div>
                    ...
                    <div class="form-control-group no-margin">
                        <div class="seo-field-heading">
                            ...
                            <span class="seo-counter-status"><span class="seo-limit-warning" data-limit-warning>Quá ký
                                    tự</span><span id="seo-description-count"
                                    class="seo-character-count">0/160</span></span>
                        </div>
                        <textarea id="seo_description" name="seo_description" class="input-text-field seo-description-field"
                            maxlength="320"
            ...
            <div class="layout-column-sidebar">
                ...
                <div class="form-card">
                    ...
                    <div class="upload-zone-wrapper" id="product-image-dropzone" role="button" tabindex="0">
                        ...
                        <input type="file" id="product-images-input" name="images[]" multiple
                    ...
                    <div id="image-upload-error"
                        class="image-upload-error {{ ($imageValidationError ?? null) ? 'visible' : '' }}" role="alert">
                        {{ $imageValidationError ?? '' }}</div>
                    ...
                    <input type="hidden" id="primary-image-id" name="primary_image_id"
                    ...
                    <div class="thumbnails-wrap-row" id="upload-thumbnails-preview">
                        @foreach($product->images->sortByDesc('is_primary') as $img)
                            ...
                            @endphp
                            <div class="thumbnail-img-box js-existing-image {{ $img->is_primary ? 'is-primary' : '' }}"
                                draggable="true" data-image-id="{{ $img->id }}" data-alt-text="{{ $img->alt_text ?? '' }}">
                                <img src="{{ $imgSrc }}" alt="{{ $imgAlt }}"
                                    onerror="this.onerror=null;this.src='{{ $fallbackImageUrl }}';">
                                @if($img->is_primary)
                                    <span class="thumbnail-primary-label">Ảnh chính</span>
                                @endif
                                <button type="button" class="btn-primary-thumb js-set-primary" title="Đặt làm ảnh chính"><i
                                        class="fa-solid fa-star"></i></button>
                                <button type="button" class="btn-delete-thumb js-delete-existing-image"
                    ...
                    <div class="image-alt-editor">
                        ...
                        <textarea id="image-alt-text" class="input-text-field" maxlength="255" disabled
                            placeholder="Chọn một ảnh để thêm mô tả SEO"></textarea>
                ...
                <div class="form-card">
                    ...
                    <div class="form-control-group">
                        ...
                        <div class="tags-input-container">
                            <span class="tag-pill">
                                Hữu cơ
                                <button type="button" class="tag-pill-btn-remove"
                                    onclick="this.closest('.tag-pill').remove();">&times;</button>
                            </span>
                            <span class="tag-pill">
                                Chó con
                                <button type="button" class="tag-pill-btn-remove"
        ...
        <div class="form-card">
            ...
            <div class="variant-builder-tools">
                <span class="variant-builder-note">Thiết lập thuộc tính trước, sau đó nhập giá và tồn kho cho từng biến
                    thể.</span>
                <div class="variant-add-menu">
                    <button type="button" id="btn-toggle-variant-add-menu" class="btn-add-variant-row" aria-expanded="false"
                        aria-controls="variant-add-menu-list">
                        <i class="fa-solid fa-plus"></i> Thêm
                    </button>
                    <div id="variant-add-menu-list" class="variant-add-menu-list" hidden>
                        <button type="button" class="variant-add-menu-item" data-add-variant="attribute"><i
                                class="fa-solid fa-sliders"></i> Thêm thuộc tính</button>
                        <button type="button" class="variant-add-menu-item" data-add-variant="variant"><i
                                class="fa-solid fa-box"></i> Thêm biến thể</button>
                    </div>
                </div>
            </div>
    ...
    <div id="product-save-modal" class="product-save-modal" hidden role="dialog" aria-modal="true"
        aria-labelledby="product-save-modal-title">
        <div class="product-save-modal-card">
            ...
            <p>{{ $isCreate ? 'Bạn có muốn tạo sản phẩm mới với các thông tin đã nhập?' : 'Bạn có muốn cập nhật thông tin sản phẩm? Dữ liệu sẽ được lưu mà không rời khỏi trang chỉnh sửa.' }}
            </p>
            <div class="product-save-modal-actions">
                <button type="button" id="btn-cancel-product-save" class="btn-action-cancel">Hủy</button>
                <button type="button" id="btn-confirm-product-save"
    ...
    <div id="image-delete-modal" class="product-save-modal" hidden role="dialog" aria-modal="true"
        aria-labelledby="image-delete-modal-title">
        <div class="product-save-modal-card">
            <div class="product-save-modal-icon"><i class="fa-solid fa-trash"></i></div>
            <h3 id="image-delete-modal-title">Xác nhận xóa ảnh</h3>
            <p>Ảnh sẽ bị bỏ khỏi sản phẩm khi lưu. Thao tác này không thể hoàn tác.</p>
            <div class="product-save-modal-actions"><button type="button" id="btn-cancel-image-delete"
                    class="btn-action-cancel">Hủy</button><button type="button" id="btn-confirm-image-delete"
                    class="btn-action-save">Xóa ảnh</button></div>
        </div>
    </div>
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (productDraftAiCard) {
                const productDraftEndpoint = @json(route('admin.products.ai.improve'));
                const generateProductDraftButton = document.getElementById('btn-generate-product-draft');
                const productDraftStatus = document.getElementById('product-draft-ai-status');
                const productDraftResult = document.getElementById('product-draft-ai-result');
                const draftLabels = { category_id: 'Danh mục', brand_id: 'Thương hiệu', short_description: 'Mô tả ngắn', description: 'Mô tả chi tiết', focus_keyword: 'Focus keyword', seo_title: 'SEO title', seo_description: 'SEO description', pet_species_ids: 'Loài thú cưng phù hợp', advice_life_stages: 'Giai đoạn sống phù hợp', advice_needs: 'Nhu cầu được hỗ trợ', variants: 'Biến thể & vận hành' };
                const setDraftStatus = (message = '', isError = false) => {
                    productDraftStatus.hidden = !message;
                    productDraftStatus.textContent = message;
                    productDraftStatus.classList.toggle('error', isError);
                };
                const displayDraftValue = (field, value) => {
                    if (field === 'category_id' || field === 'brand_id') return document.getElementById(field)?.querySelector(`option[value="${CSS.escape(String(value))}"]`)?.textContent?.trim() || String(value);
                    if (draftCheckboxNames[field] && Array.isArray(value)) return value.map((item) => {
                        const input = productEditForm.querySelector(`input[name="${draftCheckboxNames[field]}[]"][value="${CSS.escape(String(item))}"]`);
                    ...
                    if (field === 'variants' && Array.isArray(value)) return value.map((variant) => {
                        const values = (variant.value_ids || []).map((id) => variantTypes.flatMap((type) => type.values || []).find((item) => Number(item.id) === Number(id))?.value).filter(Boolean).join(' / ') || 'Biến thể mặc định';
                        const operations = [['SKU', variant.sku], ['Giá', variant.price ? `${new Intl.NumberFormat('vi-VN').format(variant.price)}đ` : ''], ['Kho', variant.quantity], ['Ship', variant.weight_grams ? `${variant.weight_grams}g` : '']].filter(([, item]) => item !== '' && item !== null && item !== undefined).map(([label, item]) => `${label}: ${item}`).join(' · ');
                        return `${values}${operations ? ` — ${operations}` : ''}${variant.operation_note ? `\n${variant.operation_note}` : ''}`;
                    }).join('\n\n');
                    if (Array.isArray(value)) return value.join(', ');
                    const node = document.createElement('div'); node.innerHTML = value || ''; return node.textContent || node.innerText || '';
                };
                let productDraftSuggestions = {};
                let productDraftSnapshot = null;
                const draftCheckboxNames = { pet_species_ids: 'pet_species_ids', advice_life_stages: 'advice_life_stages', advice_needs: 'advice_needs' };
                const snapshotDraftFields = () => {
                    if (productDraftSnapshot) return;
                    productDraftSnapshot = {
                        ...
                        focus_keyword: document.getElementById('focus_keyword')?.value || '', seo_title: document.getElementById('seo_title')?.value || '', seo_description: document.getElementById('seo_description')?.value || '',
                        category_id: document.getElementById('category_id')?.value || '', brand_id: document.getElementById('brand_id')?.value || '',
                        pet_species_ids: [...productEditForm.querySelectorAll('input[name="pet_species_ids[]"]:checked')].map((input) => input.value),
                        advice_life_stages: [...productEditForm.querySelectorAll('input[name="advice_life_stages[]"]:checked')].map((input) => input.value),
                        advice_needs: [...productEditForm.querySelectorAll('input[name="advice_needs[]"]:checked')].map((input) => input.value),
                        variants: [...document.querySelectorAll('#variants-card-list article[data-index]')].map((row) => ({
                            value_ids: JSON.parse(row.dataset.selectedIds || '[]'), sku: row.querySelector('.js-variant-sku')?.value || '', price: row.querySelector('.js-variant-price')?.value || '', sale_price: row.querySelector('.js-variant-sale-price')?.value || '', quantity: row.querySelector('.js-variant-quantity')?.value || '', weight_grams: row.querySelector('.js-variant-weight')?.value || '', status: row.querySelector('.js-variant-visible')?.checked ? 'active' : 'inactive',
                        })),
                    };
                };
                const setDraftText = (field, value) => {
                    const input = document.getElementById(field); if (!input) return;
                    input.value = value; input.dispatchEvent(new Event('input', { bubbles: true })); input.dispatchEvent(new Event('change', { bubbles: true }));
                };
                const setDraftDescription = (html) => {
                    setDraftText('description', html);
                    const syncEditor = () => {
                        if (typeof window.petworldSetProductDescription === 'function') {
                            window.petworldSetProductDescription(html);
                            return;
                        }
                        window.dispatchEvent(new CustomEvent('petworld:description-set', { detail: { html } }));
                    };
                    syncEditor();
                    window.requestAnimationFrame(syncEditor);
                };
                const setDraftCheckboxes = (name, values) => {
                    const allowed = new Set(values.map(String));
                    productEditForm.querySelectorAll(`input[name="${name}[]"]`).forEach((input) => { input.checked = allowed.has(input.value); input.dispatchEvent(new Event('change', { bubbles: true })); });
                };
                const applyDraftVariants = (variants) => {
                    const rows = [...document.querySelectorAll('#variants-card-list article[data-index]')];
                    const emptyRows = rows.every((row) => !row.querySelector('.js-variant-sku')?.value && JSON.parse(row.dataset.selectedIds || '[]').length === 0);
                    if (emptyRows) rows.forEach((row) => row.remove());
                    variants.forEach((variant) => addVariantRow(variant));
                    updateVariantsEmptyState(); updateVariantSummary(); updateProductAttributesOverview();
                };
                const applyDraftField = (field) => {
                    const value = productDraftSuggestions[field];
                    if (value === undefined || value === null || value === '' || (Array.isArray(value) && value.length === 0)) return;
                    snapshotDraftFields();
                    if (field === 'description') setDraftDescription(value);
                    else if (field === 'variants') applyDraftVariants(value);
                    else if (draftCheckboxNames[field]) setDraftCheckboxes(draftCheckboxNames[field], value);
                    else setDraftText(field, String(value));
                    setDraftStatus('Đã áp dụng đề xuất AI vào form — chưa lưu.');
                };
                generateProductDraftButton.addEventListener('click', async () => {
                    const name = document.getElementById('name')?.value?.trim();
                    if (!name) { setDraftStatus('Hãy nhập tên sản phẩm chính trước.', true); return; }
                    generateProductDraftButton.disabled = true;
                    generateProductDraftButton.textContent = 'AI đang tạo nháp...';
                    setDraftStatus('AI đang tạo bản nháp. Form hiện tại chưa thay đổi.');
                    productDraftResult.hidden = true;
                    try {
                        const response = await fetch(productDraftEndpoint, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json', Accept: 'application/json', 'X-CSRF-TOKEN': productEditForm.querySelector('input[name="_token"]')?.value },
                            body: JSON.stringify({ action: 'generate_product_draft', product: { name }, options: { length: 'detailed', tone: 'professional' } }),
                        });
                        const json = await response.json().catch(() => ({}));
                        if (!response.ok) throw new Error(json.message || 'AI chưa thể tạo nháp.');
                        productDraftResult.innerHTML = '';
                        Object.entries(json.data?.suggestions || {}).forEach(([field, value]) => {
                            if (value === null || value === '' || (Array.isArray(value) && value.length === 0)) return;
                            const item = document.createElement('div'); item.className = 'product-draft-ai-item';
                            const title = document.createElement('strong'); title.textContent = draftLabels[field] || field;
                            const preview = document.createElement('p'); preview.textContent = displayDraftValue(field, value);
                            const apply = document.createElement('button'); apply.type = 'button'; apply.className = 'product-draft-ai-apply'; apply.textContent = 'Áp dụng';
                            apply.addEventListener('click', () => { applyDraftField(field); item.remove(); });
                            item.append(title, preview, apply); productDraftResult.appendChild(item);
                        });
                        (json.data?.warnings || []).forEach((warning) => { const item = document.createElement('div'); item.className = 'product-draft-ai-item'; item.innerHTML = '<strong>Cần kiểm tra</strong>'; const preview = document.createElement('p'); preview.textContent = warning; item.appendChild(preview); productDraftResult.appendChild(item); });
                        productDraftSuggestions = json.data?.suggestions || {};
                        if (productDraftResult.children.length) {
                            const footer = document.createElement('div'); footer.className = 'product-draft-ai-footer';
                            const applyAll = document.createElement('button'); applyAll.type = 'button'; applyAll.className = 'product-draft-ai-apply'; applyAll.textContent = 'Áp dụng tất cả';
                            applyAll.addEventListener('click', () => { Object.keys(productDraftSuggestions).forEach(applyDraftField); productDraftResult.querySelectorAll('.product-draft-ai-item').forEach((item) => item.remove()); applyAll.hidden = true; });
                            const undo = document.createElement('button'); undo.type = 'button'; undo.className = 'product-draft-ai-apply'; undo.textContent = 'Hoàn tác AI';
                            undo.addEventListener('click', () => {
                                if (!productDraftSnapshot) return;
                                Object.entries(productDraftSnapshot).forEach(([field, value]) => {
                                    if (field === 'description') setDraftDescription(value);
                                    else if (field === 'variants') { document.querySelectorAll('#variants-card-list article[data-index]').forEach((row) => row.remove()); value.forEach((variant) => addVariantRow(variant)); updateVariantsEmptyState(); updateVariantSummary(); updateProductAttributesOverview(); }
                                    else if (draftCheckboxNames[field]) setDraftCheckboxes(draftCheckboxNames[field], value);
                                    else setDraftText(field, value);
                                });
                                productDraftSnapshot = null; setDraftStatus('Đã hoàn tác các thay đổi AI trên form.');
                            });
                            footer.append(applyAll, undo); productDraftResult.appendChild(footer);
                        }
                        productDraftResult.hidden = productDraftResult.children.length === 0;
                        setDraftStatus('AI đã tạo nháp. Hãy xem đề xuất trước khi áp dụng.');
                    } catch (error) { setDraftStatus(error.message || 'Không thể kết nối AI.', true); }
                    finally { generateProductDraftButton.disabled = false; generateProductDraftButton.textContent = 'Tạo nháp từ tên sản phẩm'; }
                });
                const generateDraftImageAltButton = document.getElementById('btn-generate-draft-image-alt');
                generateDraftImageAltButton.addEventListener('click', async () => {
                    const name = document.getElementById('name')?.value?.trim();
                    if (!name) { setDraftStatus('Hãy nhập tên sản phẩm chính trước.', true); return; }
                    if (!selectedImages.length) { setDraftStatus('Hãy chọn ít nhất một ảnh trước khi tạo alt bằng AI.', true); return; }
                    const formData = new FormData();
                    formData.append('action', 'generate_draft_image_alt'); formData.append('product[name]', name);
                    selectedImages.forEach((image) => { formData.append('draft_images[]', image.file); formData.append('draft_image_keys[]', image.key); });
                    generateDraftImageAltButton.disabled = true; generateDraftImageAltButton.textContent = 'AI đang đọc ảnh...';
                    setDraftStatus('AI đang tạo alt cho ảnh chưa lưu. Form chưa được lưu.');
                    try {
                        const response = await fetch(productDraftEndpoint, { method: 'POST', headers: { Accept: 'application/json', 'X-CSRF-TOKEN': productEditForm.querySelector('input[name="_token"]')?.value }, body: formData });
                        const json = await response.json().catch(() => ({}));
                        if (!response.ok) throw new Error(json.message || 'AI chưa thể tạo alt ảnh.');
                        const alts = json.data?.suggestions?.image_alts || [];
                        productDraftResult.innerHTML = '';
                        alts.forEach((item) => {
                            const card = document.createElement('div'); card.className = 'product-draft-ai-item';
                            const title = document.createElement('strong'); title.textContent = 'Alt ảnh';
                            const preview = document.createElement('p'); preview.textContent = item.alt_text;
                            const apply = document.createElement('button'); apply.type = 'button'; apply.className = 'product-draft-ai-apply'; apply.textContent = 'Áp dụng';
                            apply.addEventListener('click', () => {
                                const key = String(item.id).replace(/^new:/, '');
                                const image = selectedImages.find((entry) => entry.key === key);
                                const box = document.querySelector(`.js-new-image[data-image-key="${CSS.escape(key)}"]`);
                                if (image) image.altText = item.alt_text;
                                if (box) box.dataset.altText = item.alt_text;
                                if (box && imageToken(box) === selectedImageToken) { imageAltInput.value = item.alt_text; imageAltCount.textContent = item.alt_text.length; }
                                syncImageMetadata(); card.remove(); setDraftStatus('Đã áp dụng alt ảnh — chưa lưu.');
                            });
                            card.append(title, preview, apply); productDraftResult.appendChild(card);
                        });
                        productDraftResult.hidden = alts.length === 0;
                        setDraftStatus(alts.length ? 'AI đã tạo alt ảnh. Hãy xem và áp dụng từng ảnh.' : 'AI chưa nhận diện đủ rõ để tạo alt ảnh.');
                    } catch (error) { setDraftStatus(error.message || 'Không thể kết nối AI.', true); }
                    finally { generateDraftImageAltButton.disabled = false; generateDraftImageAltButton.textContent = 'Tạo alt ảnh bằng AI'; }
                });
            }
