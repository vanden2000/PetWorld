<aside class="product-readiness" id="product-readiness" aria-live="polite">
    <button type="button" class="product-readiness-toggle" id="product-readiness-toggle" aria-expanded="false">
        <i class="fa-solid fa-clipboard-check"></i><span id="product-readiness-summary">Kiểm tra sản phẩm</span><i class="fa-solid fa-chevron-down"></i>
    </button>
    <div class="product-readiness-panel" id="product-readiness-panel" hidden>
        <div class="product-readiness-head"><strong id="product-readiness-title">Hồ sơ sản phẩm</strong><span id="product-readiness-score">0/0</span></div>
        <p id="product-readiness-note"></p>
        <div class="product-readiness-section"><strong>Bắt buộc để lưu</strong><ul id="product-readiness-required"></ul></div>
        <div class="product-readiness-section"><strong>Nên bổ sung</strong><ul id="product-readiness-recommended"></ul></div>
        <div class="product-readiness-section" id="product-readiness-warning-section"><strong>Cần kiểm tra</strong><ul id="product-readiness-warnings"></ul></div>
    </div>
</aside>

<style>
    .product-readiness { position: fixed; top: 82px; right: 22px; z-index: 45; width: min(340px, calc(100vw - 28px)); font-size: .8rem; }
    .product-readiness-toggle { display: flex; width: 100%; align-items: center; gap: 8px; padding: 10px 12px; border: 1px solid #f0c7ad; border-radius: 10px; color: #9a3f0d; background: #fffaf6; box-shadow: 0 8px 22px rgba(151, 74, 23, .16); font: inherit; font-weight: 800; cursor: pointer; }
    .product-readiness-toggle i:last-child { margin-left: auto; transition: transform .16s ease; }
    .product-readiness.is-open .product-readiness-toggle { border-radius: 10px 10px 0 0; }
    .product-readiness.is-open .product-readiness-toggle i:last-child { transform: rotate(180deg); }
    .product-readiness-panel { max-height: min(68vh, 580px); overflow-y: auto; padding: 12px; border: 1px solid #f0c7ad; border-top: 0; border-radius: 0 0 10px 10px; background: #fff; box-shadow: 0 12px 26px rgba(151, 74, 23, .16); }
    .product-readiness-head { display: flex; align-items: center; justify-content: space-between; gap: 8px; color: var(--theme-text-main); }
    .product-readiness-head span { padding: 4px 7px; border-radius: 999px; color: #a8440d; background: #fff0e5; font-size: .72rem; font-weight: 800; }
    .product-readiness-panel > p { margin: 6px 0 12px; color: var(--theme-text-gray); line-height: 1.45; }
    .product-readiness-section { margin-top: 12px; }
    .product-readiness-section > strong { color: var(--theme-text-main); font-size: .76rem; text-transform: uppercase; letter-spacing: .03em; }
    .product-readiness-section ul { display: grid; gap: 5px; margin: 7px 0 0; padding: 0; list-style: none; }
    .product-readiness-section li { display: flex; align-items: center; gap: 7px; padding: 7px; border-radius: 6px; color: var(--theme-text-gray); background: #fafafa; cursor: pointer; }
    .product-readiness-section li i { width: 14px; text-align: center; }
    .product-readiness-section li.ok i { color: #16a34a; }
    .product-readiness-section li.missing { color: #b91c1c; background: #fef2f2; }
    .product-readiness-section li.missing i { color: #dc2626; }
    .product-readiness-section li.warning { color: #a8440d; background: #fff7ed; }
    .product-readiness-section li.warning i { color: #ea580c; }
    .product-readiness-section[hidden] { display: none; }
    @media (max-width: 900px) { .product-readiness { top: 72px; right: 12px; } }
</style>
