...
export default function OrderTrackingView({ orderId }) {
  ...
  const handleSubmitReview = async ({ rating, comment }) => {
    ...
    const result = await createReview({ order_item_id: reviewItemId, rating, comment });
    setSubmittingReview(false);

    if (!result.ok) {
      toastError(result.message || "Không thể gửi đánh giá.");
      return;
    }
  ...
  return <div className="profile-shell">
    ...
    <div className="profile-content tracking-content">
      ...
      <section className="tracking-products"><header><h2>Chi tiết sản phẩm ({order.items.length})</h2></header>{order.items.map((item) => <article key={item.id}><div className="tracking-product-image" style={{ backgroundImage: `url(${resolveProductImage(item.image)})` }}/><div><strong>{item.name}</strong>{item.variant && <span>Phân loại: {item.variant}</span>}<small>Số lượng: {item.quantity}</small><div className="tracking-product-links">{item.slug && <Link href={`/shop/${item.slug}`}>Xem sản phẩm</Link>}{order.status === "completed" && (item.review ? <span className="tracking-reviewed">★ {item.review.rating}/5 · Đã đánh giá</span> : <button type="button" onClick={() => setReviewItem(item)}>Đánh giá</button>)}</div></div><b>{money(item.price * item.quantity)}</b></article>)}</section>
