...
export default function AccountView() {
  ...
  const submitAddress = async (event) => { event.preventDefault(); if (!addressForm.province || !addressForm.ward) return toastError("Vui lòng chọn đầy đủ địa chỉ hành chính."); setBusy(true); const result = await saveAddress(addressForm, addressForm.id); setBusy(false); if (!result.ok) return showError(result); const refreshed = await getAddresses(); if (refreshed.ok) setAddresses(refreshed.data.addresses || []); setAddressForm(null); toastSuccess(result.data.message); };
